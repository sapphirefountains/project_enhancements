...
project_enhancements.TaskTreeManager = class TaskTreeManager {
    ...
    fetchData() {
        ...
        Promise.all([fetchStatusOptions, fetchTasks]).then(results => {
    ...
    renderGrid(tasks) {
        ...
        const renderTaskNode = (task, container, level) => {
            ...
            const hasPendingChange = (field) => this.pendingChanges[task.name] && this.pendingChanges[task.name][field] !== undefined;
                    ...
                    <div class="task-grid-row">
                        ...
                        <div class="task-grid-cell">
                            <select class="form-control form-control-sm task-status-select pill-select" style="width: 120px; ${statusStyle}">
                                ${this.taskStatusOptions.map(s => `<option value="${s}" ${task.status === s ? 'selected' : ''}>${s}</option>`).join('')}
                            </select>
                        </div>
    ...
    bindEvents() {
        ...
        this.wrapper.on('click', '.toggle-child-tasks', function () {
            ...
            localStorage.setItem(`collapsedTasks_${me.projectName}`, JSON.stringify(Array.from(me.collapsedTasks)));
        });
    ...
    updateLocalTaskData(taskId, field, value) {
        const findAndUpdate = (list) => {
            for (let task of list) {
                ...
                if (task.children && task.children.length && findAndUpdate(task.children)) return true;
            }
            return false;
        };
    ...
    showAssigneeDialog(link) {
        ...
        const renderAssignees = () => {
            ...
            if (task.assignees && task.assignees.length > 0) {
                const assigneeItems = task.assignees.map(a => `
        ...
        const updateLink = () => {
            const text = task.assignees && task.assignees.length ? task.assignees.map(a => a.full_name).join(', ') : 'Unassigned';
            link.text(text);
