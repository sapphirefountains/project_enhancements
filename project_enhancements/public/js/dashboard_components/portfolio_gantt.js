...
project_enhancements.dashboard_components.PortfolioGantt = class PortfolioGantt {
	...
	render_controls() {
		const controlsHTML = `
			<div class="gantt-controls-wrapper d-flex align-items-center mb-3 p-3 bg-light rounded border">
				...
				<div class="d-flex align-items-center ml-4">
					...
					<div class="dropdown check-dropdown">
						...
						<div class="dropdown-menu p-2" aria-labelledby="ganttStatusDropdown" style="min-width: 200px;">
							${this.allStatuses.map(s => `
								<div class="form-check mb-1">
									<input class="form-check-input gantt-status-cb" type="checkbox" value="${s}" id="filter-gantt-${s.replace(/\s+/g, '')}" ${this.statusFilters.includes(s) ? 'checked' : ''}>
									<label class="form-check-label" for="filter-gantt-${s.replace(/\s+/g, '')}">${s}</label>
								</div>
							`).join('')}
						</div>
					</div>
		...
		// Enhancement 1: Color Differentiation CSS Injection
		if (!document.getElementById('portfolio-gantt-styles')) {
			$('<style id="portfolio-gantt-styles">').html(`
				...
				.gantt .bar-label.master-label { font-weight: bold; fill: #fff; }
				.gantt .bar-label.project-label { font-weight: bold; fill: #fff; }
				.gantt .bar-label.task-label { font-size: 11px; fill: #333; }
			`).appendTo('head');
		}
		...
		this.wrapper.find('.gantt-status-cb').on('change', () => {
			const selected = [];
	...
	render_gantt(data) {
		...
		new Gantt(gantt_container[0], mappedItems, {
			...
			custom_popup_html: function (item) {
				...
				// Strip visual indent characters from tooltip header
