...
project_enhancements.dashboard_components.PortfolioGantt = class PortfolioGantt {
    ...
    async fetch_and_render_data() {
        ...
        try {
            ...
            if (projects.message && !projects.message.error) {
                ...
                throw new Error(projects.message ? projects.message.error : 'Unknown error fetching projects for gantt');
            }
        } finally {
            this.abortController = null;
        }
    ...
    render_gantt(projects) {
        ...
        if (!projects || projects.length === 0) {
            this.wrapper.html('<p class="text-muted text-center p-4">No active projects with start dates were found to display in the Gantt chart.</p>');
            return;
        }

        const gantt_container = $('<div class="gantt-container" style="overflow-x: auto;"></div>').appendTo(this.wrapper);
