...
project_enhancements.dashboard_components.TasksView = class TasksView {
    ...
    async fetch_task_data(projectName) {
        ...
        try {
            ...
            if (tasksResult.message && !tasksResult.message.error) {
                ...
                throw new Error(tasksResult.message ? tasksResult.message.error : 'Unknown error fetching tasks');
            }
        } finally {
            this.abortController = null;
        }
    ...
    update_view_switcher_ui() {
        this.wrapper.find('.view-switcher button').removeClass('active btn-secondary').addClass('btn-outline-secondary');
        this.wrapper.find(`.view-switcher button[data-view="${this.currentSubView}"]`).removeClass('btn-outline-secondary').addClass('active btn-secondary');
    }
        ...
        const traverseTasks = (nodeList) => {
            ...
            nodeList.forEach(task => {
                mappedTasks.push({
                    ...
                    end: task.exp_end_date || frappe.datetime.add_days(frappe.datetime.get_today(), 1),
    ...
    render_gantt_view(projectName, container) {
        ...
        const ganttWrapper = $('<div class="gantt-scroll-wrapper" style="overflow-x: auto; overflow-y: hidden; height: 100%;"></div>').appendTo(container);
        ...
        frappe.require(assets, () => {
            ...
            if (!ganttTasks || ganttTasks.length === 0) {
                container.html('<div class="p-4 text-center text-muted">No task data available to construct Gantt chart.</div>');
                return;
            }
