frappe.ui.form.on('Project Workspace', {
    refresh: function(frm) {
        // Clear the page and set the title
        frm.dashboard.wrapper.empty();
        frm.set_intro("Loading Project Details...");
        ...
        frappe.require(gantt_js_url, () => {
            // After loading the Gantt library, proceed with rendering the page
...
function render_project_workspace(frm) {
    // Get the project name from the URL route (e.g., /app/project-workspace/PROJ-0001)
    const project_name = frappe.get_route()[2];
    ...
    frappe.db.get_doc('Project', project_name).then(project_doc => {
        // Update page title with the actual project name
                ...
                <div class="col-md-4">
                    <div class="card mb-3">
                        <div class="card-header">
                            <h6 class="card-title mb-0">Project Gantt Chart</h6>
                        </div>
                        <div class="card-body project-gantt-container"></div>
                    </div>
                    <div class="card">
                        <div class="card-header">
                            <h6 class="card-title mb-0">Task Tree</h6>
                        </div>
        ...
        form_container.html('<p class="text-muted">Loading form...</p>');
        ...
        form_container.find('.form-tabs').css('border-radius', '6px'); // Style adjustment
        ...
        frm.set_intro(`Error: Could not load Project "${project_name}".`, "text-danger");
        console.error(err);
    });
}
