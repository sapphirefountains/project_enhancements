...
frappe.ui.form.on("Project Workspace", {
	refresh: function (frm) {
		...
		if (!$(`link[href="${gantt_css_url}"]`).length) {
			$("<link>", { rel: "stylesheet", type: "text/css", href: gantt_css_url }).appendTo(
				"head"
			);
...
function render_project_workspace(frm) {
	...
	if (!project_name) {
		frm.set_intro(
			"No Project specified. Please access this page from the Project Dashboard or a Project form.",
			"text-danger"
		);
		return;
	}
	...
	frappe.db
			...
			frappe.call({
				...
				callback: function (r) {
					...
					if (r.message && !r.message.error && r.message.length > 0) {
						...
						const task_tree_list = $(
							'<ul class="list-group list-group-flush"></ul>'
						).appendTo(task_tree_container);
						function render_task_node(task, parent_element, level) {
							const padding = level * 20;
							$(`<li class="list-group-item" style="padding-left: ${
								padding + 15
							}px;">
							...
							if (task.children && task.children.length > 0) {
								task.children.forEach((child) =>
									render_task_node(child, parent_element, level + 1)
								);
							}
						}
		...
		.catch((err) => {
			...
			frm.set_intro(
				`Error: Could not load Project "${project_name}". Check console for details.`,
				"text-danger"
			);
		});
}
